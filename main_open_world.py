...
def get_args_parser():
    ...
    parser.add_argument('--debug_epoch', default=False, action='store_true',help='debug during training')
    parser.add_argument('--debug_eval', default=False, action='store_true',help='debug during evaluation')
    
    parser.add_argument('--use_2007', default=1,type=int)
    ...
    parser.add_argument('--emb_per_class', default=50,type=int, help='Number of prototypes per class ')
    parser.add_argument('--momentum', default=0.1, type=float, help='Momentum for updating features ')
    
    ## Logging
    parser.add_argument('--logging_freq', default=20,type=int, help='Logging frequence')
    parser.add_argument('--eval_during_training', default=500,type=int, help='Logging frequence')
    
    ## similarity parameters
    
    parser.add_argument('--start_relabelling', default=1,type=int, help='Logging frequence')
  
  
    
  
    
    ## Predict the unmatched boxes as background or not 
    parser.add_argument('--all_background', default=False, action='store_true', help='predict the rest as background')
    parser.add_argument('--relabel', default=False, action='store_true', help='relabel pseudo-unknowns')
    parser.add_argument('--ablation', default=False, action='store_true', help='Doing ablation analysis')
    #####
    parser.add_argument('--layer_option', default=0,type=int, help='Layer on which to do the similarity')
    
  
 
  
    parser.add_argument('--double_eval', default=False, action='store_true', help='Eval by removing background')
    
    ### HYPERBOLIC
    parser.add_argument('--use_hyperbolic', default=False, action='store_true', help='Use hyperbolic distance')
    parser.add_argument('--hyperbolic_coeff', default=0.1, type=float, help='Hyperbolic coefficient ')
    parser.add_argument('--family_hyperbolic_coeff', default=0.05, type=float, help='SuperClass Regularizer coefficient ')
    parser.add_argument('--samples_per_category', default=1,type=int, help='Samples per category for the hyperbolic loss')
    parser.add_argument('--hyperbolic_temp', default=0.2, type=float, help='Objectness coefficient ')
    parser.add_argument('--hyperbolic_c', default=0.1, type=float, help='curvature coefficient ')
    parser.add_argument('--clip_r', default=3.2, type=float, help='Clip r')
    parser.add_argument('--update_freq', default=50,type=int, help='Update Poincare embedding frequency ')
    parser.add_argument('--family_regularizer', default=False, action='store_true', help='Use hyperbolic distance')
    parser.add_argument('--save_objectness', default=False, action='store_true', help='record objectness for ablation')
    parser.add_argument('--hyperbolic_mean', default=False, action='store_true', help='compare wrt to hyperbolic mean for pseudo-labels')
 

    parser.add_argument('--unknown_weight', default=1.0, type=float, help='Weight of unknowns in the classification')
    parser.add_argument('--use_focal_cls', default=False, action='store_true', help='Use focal for cls')
 
    parser.add_argument('--empty_weight', default=None, type=float, help='weight of the background or last class in the classification ')
   
   
    parser.add_argument('--ablation_save_objectness', default=False, action='store_true', help='Save embedding')
    parser.add_argument('--ablation_save_embedding',default=False, action='store_true', help='Save objectness')
    parser.add_argument('--ablation_cosine',default=False, action='store_true', help='Save cosine distance')
    parser.add_argument('--use_max_uperbound',default=False, action='store_true', help='Save objectness')
 
  
    parser.add_argument('--collect_buffer',default=False, action='store_true', help='collect buffer')

    
    parser.add_argument('--save_buffer',default=False, action='store_true', help='Save Buffer')
    parser.add_argument('--load_buffer',default=False, action='store_true', help='Save Buffer')
    
 
    
    
    parser.add_argument('--buffer_dir', default='',help='path where to save, empty for no saving')
    parser.add_argument('--relevant_matrix_dir', default='',help='path where to save, empty for no saving')
...
def main(args):
    ...
    if args.load_buffer:
            ...
            criterion.hyperbolic_emb=torch.load(args.buffer_dir,map_location=args.device )
            criterion.hyperbolic_emb.update(args)
    ...
    if args.output_dir and utils.is_main_process():
        ...
        with (output_dir / "log.txt").open("a") as f:
            format = "%Y-%m-%d %H:%M:%S "
            date_and_time = datetime.now(pytz.timezone('America/Los_Angeles'))
            f.write(str(args))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.save_buffer:
            ...
            if args.compare_family:
                relevant_matrix=torch.cat((criterion.hyperbolic_emb.poincare_family_mean ,criterion.hyperbolic_emb.poincare_mean[criterion.valid_classes]))
            else:
                relevant_matrix=criterion.hyperbolic_emb.poincare_mean[criterion.valid_classes]
            
            criterion.hyperbolic_emb.relevant_matrix=relevant_matrix
            if args.save_buffer and (epoch + 1) % args.checkpoint_period == 0:
                
                buffer_paths =  os.path.join(output_dir,f'buffer{epoch:04}.pt')
        ...
        if args.output_dir:
            ...
            if (epoch + 1) % args.lr_drop == 0 or ((epoch + 1) % args.eval_every == 0 or epoch == 0 or epoch == 1):
                
                if args.multiple_pred:
                    for pred in [25,50,75,100]:
                        if pred<=args.num_queries:
                            if args.output_dir and utils.is_main_process():
                                with (output_dir / "log.txt").open("a") as f:
                                    f.write(" \n")
                                    f.write('\n Pred per image {} : Epoch {}  \n '.format(pred,epoch))
                                    f.write(" \n")

                            test_stats, coco_evaluator = evaluate(model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args,remove_background=False,pred_per_im=pred,relevant_matrix=relevant_matrix)
                
                else:
                    if args.output_dir and utils.is_main_process():
                        with (output_dir / "log.txt").open("a") as f:
                            f.write(" \n")
                            f.write('\n First Evaluation Epoch {}  \n '.format(epoch))
                            f.write(" \n")

                    test_stats, coco_evaluator = evaluate(model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args,remove_background=False)
                    if args.double_eval:
                        if args.output_dir and utils.is_main_process():
                            with (output_dir / "log.txt").open("a") as f:
                                f.write(" \n")
                                f.write('\n Second Evaluation Epoch {}  \n '.format(epoch))
                                f.write(" \n")
                        test_stats, coco_evaluator = evaluate(
                                model, criterion, postprocessors, data_loader_val, base_ds, device, args.output_dir, args,remove_background=True)
        ...
        if args.output_dir and utils.is_main_process():
            with (output_dir / "log.txt").open("a") as f:
                ...
                f.write(" \n")
                f.write('\n Epoch {}  \n '.format(epoch))
                f.write(json.dumps(date_and_time.strftime(format)) + '\n')
                f.write(json.dumps(log_stats) + "\n")
                f.write(" \n")
                
            if args.dataset in ['owod', 'owdetr','HIERARCHICAL'] and (epoch+1) % args.eval_every == 0:
                # for evaluation logs
...
def get_datasets(args):
    ...
    if args.use_hyperbolic:
        dataset_train = OWDetection(args, args.data_root, image_set=args.train_set, transforms=make_aug_transforms(args.train_set), dataset = args.dataset)
    else:
