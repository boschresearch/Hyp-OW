...
class OWEvaluator:
    def __init__(self, voc_gt, iou_types, args=None, use_07_metric=True, ovthresh=list(range(50, 100, 5))):
        ...
        self.use_07_metric = args.use_2007
        # ForkedPdb().set_trace()
        self.args=args
        ...
        for class_label_ind, class_label in enumerate(self.voc_gt.CLASS_NAMES):
            ...
            if utils.is_main_process():
                with (self.output_dir / "log.txt").open("a") as f:
                    f.write(json.dumps(class_label + " has " + str(len(lines_by_class)) + " predictions.") + "\n")
                
            ovthresh = 50
            ovthresh_ind, _ = map(self.ovthresh.index, [50, 75])
            
            # print('use 2007 is {}'.format(self.use_07_metric))
    ...
    def summarize(self, fmt='{:.06f}'):
        ...
        if utils.is_main_process():
            with (self.output_dir / "log.txt").open("a") as f:
                    f.write(json.dumps('detection mAP50: {}'.format(mAP50)) +'\n')
                    f.write(json.dumps(('detection mAP: {} ', fmt.format(mAP)))+'\n')
                    f.write(json.dumps(('---AP50---' ))+'\n')

                    f.write(json.dumps(('Wilderness Impact: ' + str(wi)))+'\n')

                    f.write(json.dumps(('avg_precision: ' + str(avg_precision_unk)))+'\n')

                    f.write(json.dumps(('Absolute OSE (total_num_unk_det_as_known): ' + str(total_num_unk_det_as_known)))+'\n')
                    f.write(json.dumps(('total_num_unk ' + str(total_num_unk)))+'\n')
                    f.write(json.dumps(("AP50: " + str(['%.1f' % x for x in self.AP[:, o50]])))+'\n')
                    f.write(json.dumps(("Precisions50: " + str(['%.1f' % x for x in self.precs[50]])))+'\n')
                    f.write(json.dumps(("Recall50: " + str(['%.1f' % x for x in self.recs[50]])) )+'\n')

                    if self.prev_intro_cls > 0:
                        f.write(json.dumps(("Prev class AP50: " + str(self.AP[:, o50][:self.prev_intro_cls].mean())))+'\n')
                        f.write(json.dumps(("Prev class Precisions50: " + str(np.mean(self.precs[50][:self.prev_intro_cls]))))+'\n')
                        f.write(json.dumps(("Prev class Recall50: " + str(np.mean(self.recs[50][:self.prev_intro_cls]))))+'\n' )

                    f.write(json.dumps(("Current class AP50: " + str(self.AP[:, o50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls].mean())))+'\n')
                    f.write(json.dumps(("Current class Precisions50: " + str(np.mean(self.precs[50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls])) ))+'\n')
                    f.write(json.dumps(("Current class Recall50: " + str(np.mean(self.recs[50][self.prev_intro_cls:self.prev_intro_cls + self.curr_intro_cls]))))+'\n')

                    f.write(json.dumps(("Known AP50: " + str(self.AP[:, o50][:self.prev_intro_cls + self.curr_intro_cls].mean())))+'\n')
                    f.write(json.dumps(("Known Precisions50: " + str(np.mean(self.precs[50][:self.prev_intro_cls + self.curr_intro_cls]))))+'\n')
                    f.write(json.dumps(("Known Recall50: " + str(np.mean(self.recs[50][:self.prev_intro_cls + self.curr_intro_cls]))))+'\n')

                    f.write(json.dumps(("Unknown AP50: " + str(self.AP[:, o50][-1])))+'\n')
                    f.write(json.dumps(("Unknown Precisions50: " + str(self.precs[50][-1])))+'\n')
                    f.write(json.dumps(("Unknown Recall50: " + str(self.recs[50][-1])))+'\n')
